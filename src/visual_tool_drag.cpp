...
void VisualToolDrag::OnSubTool(wxCommandEvent &) {
	...
	for (auto line : selection) {
		...
		if (has_move)
			...
			SetOverride(line, "\\move", std::format("({},{},{},{})", p1.Str(), p1.Str(), start, end));
		}
	}
...
void VisualToolDrag::UpdateDrag(Feature *feature) {
	...
	if (!feature->parent)
		...
		SetOverride(feature->line, "\\move", std::format("({},{},{},{})"
...
void VisualToolDrag::OnDoubleClick() {
	...
	for (auto line : c->selectionController->GetSelectedSet()) {
		...
		if (GetLineMove(line, p1, p2, t1, t2)) {
			if (t1 > 0 || t2 > 0)
				SetOverride(line, "\\move", std::format("({},{},{},{})", (p1 + d).Str(), (p2 + d).Str(), t1, t2));
			else
				SetOverride(line, "\\move", std::format("({},{})", (p1 + d).Str(), (p2 + d).Str()));
		}
		else
